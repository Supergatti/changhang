...
import { basWaterLevelForecast ,queryDataByTypeCode} from "../../../services/products";
import {dateFormat} from "../../../utils/commonUtil"
const { Item } = Form;
const { confirm } = Modal;

export default class WaterSiteForecastDetails extends Component {
    constructor(props){
      super(props)
      this.state ={
        spinning:false,
    ...
    //详情请求事件
    opop = e => {
      this.props.history.push({ pathname: `/WaterSiteDeviationDetails/${e.id}/${e.forecastSiteName}` })
    }
     //返回请求事件
     return = e => {
    ...
    initTableData = async(pageNumber,current) =>{
      const res = await basWaterLevelForecast({page:current,size:pageNumber,forecastPid:this.state.pid})
      if(res.code == 200){
        this.setState({
